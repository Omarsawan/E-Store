...
describe('User Model', () => {
  ...
  it('signin method should expects first_name and last_name to be in the database', async () => {
    await expectAsync(
      store.authenticate('does not exist', 'user', 'secretpass')
    ).toBeRejectedWithError('name is not valid');
  });

  it('signin method should expects password to be valid', async () => {
    await expectAsync(
      store.authenticate('test', 'user', 'notsecretpass')
    ).toBeRejectedWithError('password is not valid');
  });
});
...
describe('User endpoints', () => {
  ...
  beforeAll(async function () {
    const connection = await Client.connect();
    const sql =
      'DELETE FROM users; ALTER SEQUENCE users_id_seq RESTART; DELETE FROM users;';
    await connection.query(sql);
    connection.release();
    // @ts-ignore
