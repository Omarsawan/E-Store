...
const create = async (req, res) => {
    try {
        const neworder = await store.create((0, auth_1.getUserIdFromToken)(req, res));
        res.json(neworder);
    }
    catch (err) {
        if (err instanceof Error) {
...
const addProduct = async (req, res) => {
    ...
    try {
        const addedProduct = await store.addProduct(quantity, orderId, productId, (0, auth_1.getUserIdFromToken)(req, res));
