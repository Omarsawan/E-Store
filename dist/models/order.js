...
class OrderStore {
    ...
    async addProduct(quantity, order_id, product_id, user_id) {
        ...
        if (active_order.status !== 'active') {
            conn.release();
            throw new Error(`Could not add product ${product_id} to order ${order_id} because order status is ${active_order.status}`);
        }
        if (user_id != active_order.user_id) {
            throw new Error(`Logged in user can not add product to this order with id: ${order_id}`);
        }
